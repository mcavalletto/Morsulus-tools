...
sub install {
  ...
  if (! -e $path) {
    printf "Installing $what with mode 0%03o at\n  $path\n", $mode;
  } else {
    printf "Installing $what with mode 0%03o over existing file at\n  $path\n", $mode;
    if (-w $path) {
      confirm_if_interactive("Is it okay to overwrite existing file?");
      ...
      confirm_if_interactive("Is it okay to delete existing file first?");
      die "\nUnable to complete installation" unless (/^y/i);
